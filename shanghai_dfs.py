...
# 搜索路径（深度优先搜索）
def dfs_search(adjacency_list, from_station, to_station, visited=None):
    if visited is None:
        visited = set()  # 记录访问过的站点

    if from_station == to_station:
        return [from_station]

    visited.add(from_station)
    print(f"Visiting {from_station}")  # 调试信息

    for neighbour in adjacency_list[from_station]:
        if neighbour not in visited:
            path = dfs_search(adjacency_list, neighbour, to_station, visited)
            if path:
...
if __name__ == "__main__":
    ...
    if from_station not in adjacency_list or to_station not in adjacency_list:
        ...
        path = dfs_search(adjacency_list, from_station, to_station)
        if path:
